...
- 自动变量分类，以及在 GUI 中通过下拉更换、确认和恢复自动类别；发布后的项目类别优先于协议默认值。
- 顺序映射、任意层级文件夹映射、左右耳、单耳、照片分耳侧和空耳。
- 真实缩略图检查、点击原图、单槽位替换/清空、两槽位交换、当前或全部用户左右耳互换、按当前用户或全部用户应用设备照片顺序、恢复自动映射。
- 未使用/补拍照片按用户归类，可快速识别设备和耳侧并命名为已有或新视角；该规则随项目保存。
